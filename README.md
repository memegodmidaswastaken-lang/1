...
## Install into an existing Forge 1.20 template folder

If you already have a working Forge 1.20 template folder, copy all DominionCore mod source/resource files into it with:

- `bash scripts/install_into_forge_template.sh /absolute/path/to/your/forge-template`

This copies:

- Java sources **and tests** into your template package root folders when `ExampleMod.java` is found (e.g. `src/main/java/com/example/examplemod/(files)` and `src/test/java/com/example/examplemod/(files)`), otherwise into `src/main/java/dev/dominioncore/**` and `src/test/java/dev/dominioncore/**`
- `src/main/resources/dominioncore/**`
- `src/main/resources/META-INF/mods.toml`
- `src/main/resources/pack.mcmeta`
- updates template `ExampleMod.java` in-place with DominionCore Forge entrypoint content (same package) when found

After copying, open your template and adjust `mods.toml` metadata (`modId`, version, etc.) to match your project.
